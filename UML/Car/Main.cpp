...
class Engine
{
	//Ётот класс описывает двигатель
...
class Car
{
	Tank tank;
	Engine engine;
	unsigned int speed;	//текуща€ скорость
...
public:
	...
	{
		driver_inside = true;
		control.panel_thread = thread(&Car::panel, this);	//«апускаем метод panel в потоке
	}
	void get_out()
	{
		if (speed > 20) throw std::exception("You left your car on high speed, you need a doctor.");
		driver_inside = false;
		if (control.panel_thread.joinable())control.panel_thread.join();	//ќстанавливаем поток, отображающий панель приборов
	}
	void panel() const
	{
		while (driver_inside)
	...
	void engine_idle() //расход на холостом ходу.
	{
